<?php
/*文件名称: onload_collect_order.php
 *文件作用: 第一次进入领单页面或者点击了上面四个订单状态得任意一个
*/
	...
	include ("conn.php");
 	...
 	WHERE ad.user_id = od.receiver_user_id and od.courier_user_id = '$user_id' and od.order_status = '$active_index'
 	ORDER BY od.arrive_time DESC LIMIT $need_all_num
 	";
 	...
 	if (mysqli_num_rows($result) > 0) {
 		while($row = mysqli_fetch_assoc($result)) {
 			...
 			$item->show = true;
 			$pre = sprintf("%02d",$row['product_code_pre']);
 			$mid = $row['product_code_mid'];
 			$post = sprintf("%04d", $row['product_code_post']);

 			$item->code = $pre.'-'.$mid.'-'.$post;
 	...
 	$itemNum = array();
 	$sql0 = "SELECT * FROM place_order WHERE courier_user_id = '$user_id' and order_status = '0'";
 	$result0 = mysqli_query($conn, $sql0);
 	$itemNum[0] = mysqli_num_rows($result0);

 	$sql1 = "SELECT * FROM place_order WHERE courier_user_id = '$user_id' and order_status = '1'";
 	$result1 = mysqli_query($conn, $sql1);
 	$itemNum[1] = mysqli_num_rows($result1);

 	$sql2 = "SELECT * FROM place_order WHERE courier_user_id = '$user_id' and order_status = '2'";
 	$result2 = mysqli_query($conn, $sql2);
 	$itemNum[2] = mysqli_num_rows($result2);

 	$sql3 = "SELECT * FROM place_order WHERE courier_user_id = '$user_id' and order_status = '3'";
